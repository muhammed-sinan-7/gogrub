...
const ProductDetailView = ({ productId, onBack }) => {
  ...
  const uploadToCloudinary = async () => {
    ...
    try {
      const res = await cloudinaryApi.post(`/v1_1/${CLOUDINARY_CLOUD_NAME}/image/upload`, data);
      return res.data.secure_url;
    } catch (err) { return null; } finally { setImageUploading(false); }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        if (!uploadedUrl) { toast.error("Image upload failed"); return; }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center justify-between mb-8 border-b border-slate-200 pb-5">
        <button onClick={onBack} className="flex items-center gap-2 text-slate-900 font-bold hover:text-orange-600 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div className="p-6">
              ...
              <p className="text-orange-600 text-xl font-black mt-1">₹{product.price}</p>
              <div className="mt-4 p-4 bg-slate-50 rounded-2xl">
                <p className="text-slate-500 text-sm italic">"{product.description || 'No description provided.'}"</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-7">
          <div className={`bg-white rounded-3xl p-8 border transition-all duration-300 ${isEditing ? 'border-orange-200 shadow-xl' : 'border-slate-100'}`}>
            <h2 className="text-lg font-black text-slate-900 mb-6 uppercase tracking-tight">Configuration</h2>
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-1 gap-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Category ID</label>
                    <input
                      type="number"
                      name="category"
                      disabled={!isEditing}
                ...
                <div>
                  <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Full Description</label>
                  <textarea
                    name="description"
                    disabled={!isEditing}
              ...
              {isEditing && (
                <div className="pt-2 animate-in slide-in-from-top-2">
                  <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Change Image</label>
                  <div className="mt-2 flex items-center gap-4">
                    <label className="flex-1 border-2 border-dashed border-slate-200 rounded-xl p-4 flex flex-col items-center justify-center cursor-pointer hover:bg-slate-50 transition-colors">
                      ...
                      <span className="text-[10px] font-bold text-slate-400 uppercase">Upload New File</span>
              ...
              <div className="flex items-center gap-6 pt-2">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" name="is_special" checked={formData.is_special} onChange={handleChange} disabled={!isEditing} className="w-4 h-4 rounded text-orange-600 focus:ring-orange-500 border-slate-300" />
                  <span className="text-xs font-bold text-slate-700 uppercase">Special Item</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" name="is_available" checked={formData.is_available} onChange={handleChange} disabled={!isEditing} className="w-4 h-4 rounded text-black focus:ring-black border-slate-300" />
                  <span className="text-xs font-bold text-slate-700 uppercase">Visible in Menu</span>
                </label>
              </div>

              {isEditing && (
                <div className="flex justify-end gap-3 pt-8 border-t border-slate-100">
                  <button onClick={cancelEdit} className="flex items-center gap-1 text-xs font-bold text-slate-400 hover:text-red-500 transition-colors px-4 py-2">
                    <X size={14} /> Discard
                  </button>
                  <button
                  ...
                  >
                    {updating || imageUploading ? <Loader2 className="animate-spin" size={14} /> : <Save size={14} />}
                    Save Changes
                  </button>
                </div>
              )}
